...
const filtered = computed<ProjectModel[]>(() => {
  ...
  return store.projects.filter((p) => {
    const haystack = [p.full_name, p.sort_name, p.code, p.main_constructor, p.visibility_label]
      .filter(Boolean)
      .join(' ')
      .toLowerCase();
    ...
    <v-col cols="12" md="10" lg="8" class="pa-0">
      <PullToRefresh :on-refresh="onPullRefresh" :disabled="store.isLoading">
        ...
        <template v-if="store.isLoading && !store.hasProjects">
          <v-card v-for="n in 4" :key="n" class="mb-3 pa-3" elevation="1">
            ...
            <div class="text-body-2 text-medium-emphasis mt-1">
              {{
                search
                  ...
                  : 'Pull to refresh once projects are available.'
              }}
            </div>
          </v-card>
        </template>
          ...
          >
            <div class="project-cover">
              ...
              >
                ...
                <template #error>
                  <div
                    class="project-cover__fallback d-flex fill-height align-center justify-center"
                  >
                    <v-icon size="56" color="white"> mdi-image-broken-variant </v-icon>
                  </div>
                </template>
              </v-img>
              <div v-else class="project-cover__fallback d-flex align-center justify-center">
            ...
            <v-card-text class="pt-2">
              <div v-if="project.main_constructor" class="d-flex align-center mb-1 text-body-2">
                <v-icon size="18" class="mr-2" color="medium-emphasis">
              ...
              <div class="d-flex align-center flex-wrap text-body-2">
                <v-icon size="18" class="mr-2" color="medium-emphasis"> mdi-calendar-range </v-icon>
                <span class="text-medium-emphasis mr-2">:</span>
                <span
